...
const FavouriteButton = ({data}: FavouriteButtonProps) => {
    ...
    const handleToggleFavourite = () =>{
        if(isCurrentlyFavourite){
            ...
            toast.error(`Removed ${data.name} from Favoutites`)
        }else{
            ...
            toast.success(`Added ${data.name} to Favoutites`)
        }
    }

  return (
    <Button variant={isCurrentlyFavourite ? "default" : "outline"} size={"icon"} className={isCurrentlyFavourite ? "bg-yellow-500 hover:bg-yellow-600" : ""} onClick={handleToggleFavourite}>
        <Star 
